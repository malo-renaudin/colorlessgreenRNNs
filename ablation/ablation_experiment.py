...
sys.path.append(
    os.path.abspath(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../src/word_language_model"
        )
    )
)
import data
...
parser.add_argument(
    "-i", "--input", required=True, help="Input sentences in Tal's format"
)
...
parser.add_argument(
    "-u", "--unit", type=int, action="append", help="Which test unit to ablate"
)
parser.add_argument(
    "-uf",
    "--unit-from",
    type=int,
    default=False,
    help="Starting range for test unit to ablate",
)
parser.add_argument(
    "-ut",
    "--unit-to",
    type=int,
    default=False,
    help="Ending range for test unit to ablate",
)
parser.add_argument(
    "-s", "--seed", default=1, help="Random seed when adding random units"
)
parser.add_argument(
    "-g",
    "--groupsize",
    default=1,
    help="Group size of units to ablate, including test unit and random ones",
)
...
for unit_group in tqdm(target_units):
    ...
    if unit_group:
        ...
        add_random_subset = np.random.permutation(1301).astype(int)
        units_to_kill = add_random_subset[
            0 : (int(args.groupsize))
        ]  # add g random units
        unit_group = ["CONTROL"] + list(units_to_kill)

    units_to_kill = [u - 1 for u in units_to_kill]  # Change counting to zero
    units_to_kill_l0 = torch.LongTensor(
        np.array([u for u in units_to_kill if u < 650])
    )  # units 1-650 (0-649) in layer 0 (l0)
    units_to_kill_l1 = torch.LongTensor(
        np.array([u - 650 for u in units_to_kill if u > 649])
    )  # units 651-1300 (650-1299) in layer 1 (l1)
    if args.cuda:
    ...
    print(
        "ablated units: ",
        units_to_kill_l0.cpu().numpy() + 1,
        units_to_kill_l1.cpu().numpy() + 651,
    )
    print("\n\n\n")
    ...
    for ablation in [args.do_ablation]:  # [False, True]:
        if ablation:
            ...
            if len(units_to_kill_l0) > 0:
                model.rnn.weight_hh_l0.data[:, units_to_kill_l0] = (
                    0  # l0: w_hi, w_hf, w_hc, w_ho
                )
            if len(units_to_kill_l1) > 0:
                model.rnn.weight_hh_l1.data[:, units_to_kill_l1] = (
                    0  # l0: w_hi, w_hf, w_hc, w_ho
                )
            ...
            if len(units_to_kill_l1) > 0:
                model.decoder.weight.data[:, units_to_kill_l1] = 0
        ...
        for i, s in enumerate(tqdm(sentences)):
            ...
            for j, w in enumerate(s):
                ...
                if j == gold.loc[i, "verb_pos"] - 1:
                    ...
                    log_p_targets_correct[i] = out[
                        0, 0, vocab.word2idx[gold.loc[i, "correct"]]
                    ].data.item()
                    log_p_targets_wrong[i] = out[
        ...
        print(
            "p_difference: %1.3f +- %1.3f"
            % (score_on_task_p_difference, score_on_task_p_difference_std)
        )
        # Save to file
        if args.format == "npz":
            np.savez(output, **out)
