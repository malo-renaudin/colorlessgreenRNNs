...
def permute_tokens(token_string, seed=12345):
    ...
    #token_list = token_string.strip(".").split()#word_tokenize(token_string.strip("."))
    stripped_string = token_string.strip(".")
    # Pattern: \w+ matches one or more word characters (alphanumeric + underscore)
    #          [^\s\w] matches a single character that is NOT whitespace and NOT a word character (i.e., punctuation)
    token_list = re.findall(r"\w+|[^\s\w]", stripped_string)
    first = token_list[0]
...
if __name__ == "__main__":
    ...
    savedir = '/scratch2/mrenaudin/colorlessgreenRNNs/wm_tests'
