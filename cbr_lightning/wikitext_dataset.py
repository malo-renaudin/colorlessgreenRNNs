...
class WordTokenizer:
    ...
    def build_vocab(self, texts: List[str]) -> None:
        ...
        print(f"Special tokens: {len(self.special_tokens)} -> {self.special_tokens}")
        ...
        for token in self.special_tokens:
            ...
            print(f"Special token: '{token}' -> ID {current_id}")
            current_id += 1
        
        # Add vocabulary words, ensuring we don't exceed vocab_size
        words_added = 0
        max_words = self.vocab_size - len(self.special_tokens)
        
        for word, count in most_common:
            if count >= self.min_freq and current_id < self.vocab_size:
        ...
        max_token_id = max(self.word2idx.values()) if self.word2idx else -1
        
        print(f"Words added to vocabulary: {words_added}")
        print(f"Final vocabulary size: {actual_vocab_size}")
        print(f"Maximum token ID assigned: {max_token_id}")
        print(f"Valid token ID range: 0 to {max_token_id}")
        
        # Show most common words that made it into vocabulary
        vocab_words = [word for word, _ in most_common 
                      if word in self.word2idx and word not in self.special_tokens][:10]
        print(f"Most common words in vocabulary: {vocab_words}")
        
        # Verify all token IDs are valid (should be 0 to actual_vocab_size-1)
        for word, token_id in self.word2idx.items():
            if token_id < 0 or token_id >= actual_vocab_size+1:
                raise ValueError(f"Invalid token ID {token_id} for word '{word}' (valid range: 0-{actual_vocab_size-1})")
        
        print("✓ Vocabulary building completed successfully")
    def encode(self, text: str) -> List[int]:
        ...
        for token in tokens:
            if token in self.word2idx:
                ...
                # Safety check: ensure token ID is within vocabulary bounds
    ...
    def __init__(
        ...
        for article_idx, text in enumerate(texts):
            if text.strip():
               ...
               # Create sliding window sequences WITHIN this article only
               if len(tokens) >= self.sequence_length + 1:
                  if self.sliding_window:
                    # Sliding window with stride
                    for i in range(0, len(tokens) - self.sequence_length, self.window_stride):
                        if i + self.sequence_length + 1 <= len(tokens):
                            sequence = tokens[i:i + self.sequence_length + 1]
                            self.sequences.append(sequence)
               else:
                    # Non-overlapping chunks within article
        ...
        print(f"Sequences respect article boundaries - no cross-article context")
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = WikiTextDataset(
                ...
                self.add_special_tokens, self.sliding_window, self.window_stride
            )
