...
parser.add_argument(
    "model",
    type=str,
    default="/scratch2/mrenaudin/colorlessgreenRNNs/checkpoints/rnn_relu/epoch_40.pt",
...
parser.add_argument(
    "-v", "--vocabulary", default="/scratch2/mrenaudin/colorlessgreenRNNs/english_data"
)
parser.add_argument(
    "-o",
    "--output",
    default="results_rnn_relu",
...
parser.add_argument("--format", default="csv", choices=["npz", "hdf5", "pkl", "csv"])
...
parser.add_argument(
    "--batch-size", type=int, default=1, help="Batch size for processing sentences"
)
...
# model.forward = lambda input, hidden: lstm.forward(model.rnn, input, hidden)
...
if args.format == "npz":
    ...
    # Convert the results to a pandas DataFrame and save it as CSV
    result_df = pandas.DataFrame(
        {
            "log_p_targets_correct": log_p_targets_correct.flatten(),
            "log_p_targets_wrong": log_p_targets_wrong.flatten(),
            "accuracy_score_on_task": [score_on_task] * len(sentences),
            "p_difference": p_difference.flatten(),
            "score_on_task_p_difference": score_on_task_p_difference,
            "score_on_task_p_difference_std": score_on_task_p_difference_std,
            "sentences": [" ".join(s) for s in sentences],
            "nattr": gold["nattr"],
            "verb_pos": gold["verb_pos"],
            "conditions": conditions,
        }
    )
