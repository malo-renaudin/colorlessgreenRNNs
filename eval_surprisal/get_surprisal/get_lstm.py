        ...
        if args.aligned:
            ...
            for i, (word, pieces) in enumerate(zip(words, piecess)):
                ...
                for merge_fn, merge_f in merge_fs.items():
                    new_row[merge_fn +  "surprisal"] = merge_f(surps)
                    
                    # Surprisal in bits from base-change formula
                    new_row[merge_fn +  "surprisal_base2"] = merge_f(surps_b2)
            ...
            for i, (word_idx, word) in enumerate(zip(input, sentence[1:])): # drop EOS
                ...
                new_row["surprisal_b2"] = -F.log_softmax(out[i], dim=-1).view(-1)[word_idx].item()/np.log(2.0)
...
# python eval_surprisal/get_surprisal/get_lstm.py --input eval_surprisal/get_surprisal/data/items_RelativeClause.pivot.csv --out eval_surprisal/get_surprisal/data/items_RelativeClause.lstm.csv --vocab_path english_data --model /scratch2/mrenaudin/colorlessgreenRNNs/checkpoints/lstm_adam_full_check_shuffled/epoch_40.pt  --aligned 
#python eval_surprisal/get_surprisal/postprocess.py --input eval_surprisal/get_surprisal/data/items_ClassicGP.lstm.csv --output eval_surprisal/get_surprisal/data/items_ClassicGP.lstm.post.csv
